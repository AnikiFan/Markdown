...
数组可以理解为一种特殊的变量，它能够作为数组的元素。当我们对它进行加减法时，则它会转化为基类型为数组的基类型的指针，同样的，它也可以赋值给
基类型为它的基类型的指针变量， `*`作用在它上时，实际上也是先将他转化为指针，然后再作运算。因为数组是变量，所以可以
通过`[]`来作为数组元素获取，或者通过指向它的指针来获取，但是无法通过`&`来作为地址获取。
...
1. 不能主动释放非动态内存申请的空间
2. 当一个指针中存储的是由动态内存申请得到的地址时，如果释放了该指针，那么释放后该指针中存储的地址会被更换为随机分配的地址。所以当二次释放同一指针时，实际上会释放一个不是经过动态内存申请的空间，从而会报错
...
1. 返回类型： 要根据实际用途以及原生的运算符的性质来确定，例如`=`，它的返回类型不为`void`，而是所赋的值。如果是返回对象自身的话，当涉及到动态内存申请时要小心。如果返回的不是引用类型，则有可能调用复制构造函数，如果是默认的复制构造函数话，便会造成内存错误。同时，如果返回值是可以修改的左值，例如`a=b`的返回值其实是`a`，可以通过`(a=b)++`来修改`a`的值，那么在编写对应的重构函数时，其返回值应该是被赋值的变量的引用。如果返回的不是可以修改的左值，则需要返回的是临时变量，即不能返回引用。有些时候，由于不涉及动态内存申请，形参是否使用引用形式并不是必要的，但是一般推荐使用引用形式，因为这样可以避免内存间频繁的拷贝。
...
注意，在编写自定义类型的前后缀`++`,`--`时，友元函数方式实现时前缀对应的形参表中只有自身，而后缀的形参表中除了自身外还有一个`int`型，不进行访问，只用于区分。当用成员函数方式进行实现时，则两者第一个形参，即自身，都不用写，但是后缀的形参表中仍然有`int`来进行区分。
...
`cin`对应着 `istream`，`cout`对应着 `ostream`

由于`cin`,`cout`总是在`<<`或`>>`的左侧，所以形参表的顺序不可变。

### 类型转换 

下面两类类型转换都是为了让自定义类型在运算时拥有类似于普通类型类型转换的功能。

#### 将其他类型变量转换为自定义类型

主要用于解决运算时的类型转换以及构造时的类型转换问题。
如果想将其他类型转换为自定义类型，那么只需要为该自定义类型增加一个单形参构造函数即可，这种类型的构造函数被称为转换构造函数。（实际上是人为根据构造函数的实际功能给出的分类）。

可以通过`类名(要转换类型的数据)`或 `对象名=要转换的数据`来进行调用

#### 将自定义类型转换为其他类型变量

主要用于解决运算时的类型转换。只能用成员函数的形式实现，无形参。
...
如果在一个阶段中找不到匹配的函数，则到下一阶段。如果任意一个阶段中有多个匹配的函数，则报错。
当重载运算符时，例如`+`，那么自定义的重载运算符和原先系统自带的运算符实际上是两个重载函数，也遵循以上规则，如果我们定义了`自定义类+整数`以及自定义类转换为整数的转换函数，则`自定义类+自定义类`会有二义性，因为在第三阶段，既可以转换后一个自定义类，使得形参表满足第一个函数，又可以转换两个自定义类，来满足系统的加法。
...
单继承的继承关系图是树状结构。当存在多级派生的现象时，权限逐级确定。

在分析多重构造函数的调用顺序时，实际上是倒置的树的深度优先遍历。

实际使用中，用于派生的基类的成员属性不使用`private`，继承方式使用`public`，这样能保证各个成员的访问权限不会随着继承而改变。

### 同名问题

当派生类成员与基类成员同名时，派生类优先于基类，如果想要访问基类的成员，则需要使用`::`运算符。

不同层次之间的函数间不存在重载，需要通过`::`来区分层次。

当通过直接/间接方式继承同一个基类而产生两个同名时，需要通过可以区分的类的作用域符来区分。

### 虚基类 

当出现一个类的多个直接基类有共同间接或直接基类时，该类中会有这个类的成员的多份拷贝，如果只想要有一份，则需要使用虚基类。

只有一个类的派生类都加上`virtual`限定符时，才能保证以这些派生类作为基类的派生类中不会出现最初的类的成员的多份拷贝。此时，相当于把调用最初这个类的权限交给了间接基类，而非多个直接基类。为此，需要在间接派生类的形参表后激活最初这个类的构造函数，否则调用无参构造。

虚基类这个概念只在多级继承时有用，在单级继承中无用。

### 构造函数

构造函数无法继承，同时派生类中又含有基类的成员，需要同时初始化基类和派生类的数据成员，由此需要在派生类的构造函数中激活直接基类的构造函数。
...
多重继承时调用基类的构造函数顺序时按照继承时的声明顺序，而不是形参表的顺序，内存中成员存储顺序也是和声明顺序一致。派生类的构造函数最后调用，自身的成员也永远在最后。

派生类构造函数不调用基类的构造函数时，调用的是基类的无参构造

什么类中的成员涉及动态内存申请，则该类的构造函数是必要的。
...
派生类能够自动转化到其公有继承的基类，这是系统定义的转化之一。系统会自动截取出派生类中属于给定基类的那一部分成员。这一转化可以在任何需要基类对象的位置出现。

1. 派生类对象可以初始化基类对象或引用
2. 派生类对象可以出现在函数参数/返回值为基类的地方
3. 派生类对象可以赋值给基类 
4. 派生类对象的指针可以出现在基类指针出现的位置
...
1. 作为成员的类要先声明
2. 成员类的构造函数和基类的构造函数一样，跟在形参表后面激活

当形参表不一致时，失去多态性，但是如果继承序列中有其他的形参表和基类形参表一致的话，多态性仍保持，但是对于那些不同的，多态性边失去了。

虚析构函数的逻辑也是突破限制，调用派生类的析构，而基类虚构被调用了是因为，派生类析构会激活基类虚构，与虚函数无关。
...
面向对象方法学的定义为：不同对象能接受同一信息，并自主地对消息进行处理

程序设计语言+类 = 基于对象的程序设计语言
程序设计语言+类+多态 = 面向对象的程序设计语言

C++中多态的含义即：同一作用域内有多个不同功能的函数可以具有相同的函数名。
...
虚函数解决的是“在类的继承层次中，对于参数个数、参数类型完全相同的同名函数”的访问问题。

将基类中的同名函数定义前加`virtual`声明，便能使得基类引用和基类指针在接受派生类对象时，所指的函数便是所给派生类对应的函数，打破了赋值兼容规则，使得基类指针不再只能指向基类中的同名函数。

若派生类中没有同名函数，则会沿着基类向上搜索同名函数。

若基类没有同名函数，则常常使用空虚函数以及抽象类进行解决。

当使用虚函数后，如果继承序列中的同名函数出现了参数个数、类型完全相同，仅仅返回类型不同，则会报错。

如果出现了参数个数、或类型不相同的情况，则对应类在该继承序列中不会产生虚函数的效果，但是其他满足的类仍会产生。
...
虚析构函数即一种特殊的虚函数，用于解决“通过基类指针动态申请派生类对象时，申请时调用了派生类的构造函数，而释放时只调用了基类的析构函数”这一问题。

为此，将继承序列中的所有析构函数加上`virtual`声明，虽然不同类的析构函数的标识符不同，但是仍然视为虚函数。从而可以突破赋值兼容的约束，使得基类指针能够指向派生类的析构函数。而调用了派生类的析构函数后，又会自动激活基类的析构函数，从而解决了之前提出的问题。

### 空虚函数

为了使用虚函数功能，有些时候，即使基类并没有同名函数，但是也可以写一个同名空体函数，来满足虚函数的使用要求。

此时这个基类的其他部分仍可以有实际意义

### 抽象类 

目的也是为了使用虚函数，但是抽象类不能够产生实例，它只是单纯用于辅助，在更高的层次上进行统一。可以有成员变量以及函数，但是其中必须要有至少一个纯虚函数。可以有空虚函数及其他函数。
...
如果基类中的同名函数是纯虚函数，那么继承序列中如果想调用同名函数，必须要有自己的。而如果基类中的是空虚函数，那么继承序列中的类中即使没有同名函数，也会调用基类中的空虚函数。